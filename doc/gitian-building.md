...
*Setup instructions for a Gitian build of Graviocoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Graviocoin
...
- [Building Graviocoin Core](#building-graviocoin-core)
...
Clone the git repositories for graviocoin and Gitian.
...
in the graviocoin repository under 'Fetch and create inputs' to install sources which require
...
To build Graviocoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the graviocoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/graviocoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/graviocoin-project/graviocoin
...
For example:
```bash
URL=https://github.com/thrasher-/graviocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit graviocoin=${COMMIT} --url graviocoin=${URL} ../graviocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit graviocoin=${COMMIT} --url graviocoin=${URL} ../graviocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit graviocoin=${COMMIT} --url graviocoin=${URL} ../graviocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the graviocoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../graviocoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/graviocoin
SIGPATH=/some/root/path/graviocoin-detached-sigs

./bin/gbuild --url graviocoin=${BTCPATH},signature=${SIGPATH} ../graviocoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/graviocoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/graviocoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/graviocoin-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/graviocoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
